...
st.title("Мини-приложение 'Crypto Price'")
st.markdown(
    """
Это ОБУЧАЮЩЕЕ приложение предоставляет текущие цены на криптовалюту из ТОП-100 **CoinMarketCap**
"""
)
# ---------------------------------#
# О проекте
expander_bar = st.expander("О проекте")
expander_bar.markdown(
    """
* **Библиотеки Python:** base64, pandas, streamlit, numpy, matplotlib, seaborn, BeautifulSoup, requests, json, time
* **Источник:** [CoinMarketCap](http://coinmarketcap.com).
* **Предоставлено:** Парсер был взят на основе статьи из Medium *[Web Scraping Crypto Prices With Python](https://towardsdatascience.com/web-scraping-crypto-prices-with-python-41072ea5b5bf)*, автор: [Bryan Feng](https://medium.com/@bryanf).
"""
)
...
# ---------------------------------#
# Sidebar + Main panel
col1.header("Параметры:")

# Sidebar - Currency price unit
currency_price_unit = col1.selectbox("Выберете валюту для оценки", ("USD", "BTC", "ETH"))
...
selected_coin = col1.multiselect("Выбор криптовалюты", sorted_coin, sorted_coin)

df_selected_coin = df[(df["coin_symbol"].isin(selected_coin))]  # Filtering data

# Sidebar - Number of coins to display
num_coin = col1.slider("Показывать первые N названий", 1, 100, 100)
df_coins = df_selected_coin[:num_coin]

# Sidebar - Percent change timeframe
percent_timeframe = col1.selectbox("Временной промежуток", ["7d", "24h", "1h"])
percent_dict = {
...
sort_values = col1.selectbox("Использовать сортировку?", ["Yes", "No"])

col2.subheader("Данные по криптовалюте:")
col2.write(
    "Размер таблицы: "
...
def filedownload(df):
    ...
    href = f'<a href="data:file/csv;base64,{b64}" download="crypto.csv">Загрузить CSV-файл</a>'
    return href
...
col2.subheader("Изменение стоимости в %")
df_change = pd.concat(
    [
...
col3.subheader("График изменения стоимости в %")

if percent_timeframe == "7d":
    if sort_values == "Yes":
        df_change = df_change.sort_values(by=["percent_change_7d"])
    col3.write("*За 7 дней*")
